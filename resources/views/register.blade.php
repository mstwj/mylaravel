...
<script type="text/javascript">
 ...
 $('#x12').next().hide();
  ...
  $('.bk_phone_code_send').click(function(event){
    ...
    if(phone == '') {
      $('.bk_toptips').show();
      $('.bk_toptips span').html('请输入手机号-验证码功能');
    ...
    $(this).removeClass('bk_important'); //--移除一个字体.
    $(this).addClass('bk_summary'); //添加新字体.
    ...
    var interval = window.setInterval(function() {
      ...
      if (num == 0) {
        $('.bk_phone_code_send').removeClass('bk_summary'); //--移除一个字体.
        $('.bk_phone_code_send').addClass('bk_important'); //添加新字体.
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if(data.status != 0) {
          ...
          console.log(data);
          $('.bk_toptips').show();
          $('.bk_toptips span').html(data.message[0]); //{0:"xxxx"}
          setTimeout(function() {$('.bk_toptips').hide();}, 2000);
          return;
        }
